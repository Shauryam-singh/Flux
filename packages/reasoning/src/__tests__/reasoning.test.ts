...
describe("DefaultReasoningEngine", () => {
  it("should not reason when no triggers and has history", () => {
    ...
    engine["thoughtHistory"] = [{ id: "t1", type: "concern", content: "seed", confidence: 0.5, reasoning: "", timestamp: Date.now(), relatedGoalId: null, relatedObservationIds: [], suggestedAction: null }];
    expect(engine.shouldReason(
      { project: null, application: DEFAULT_APPLICATION_STATE, system: DEFAULT_SYSTEM_STATE, timestamp: Date.now(), version: 0 },
      { entries: [], totalWeight: 0, capacity: 50, utilization: 0, timestamp: Date.now() },
      [],
    )).toBe(false);
  });
    ...
    expect(engine.shouldReason(
      {
        ...
        system: { ...DEFAULT_SYSTEM_STATE, openErrors: [{ source: "build", message: "fail", timestamp: Date.now() }] },
  ...
  it("should reason when goals are blocked", () => {
    ...
    expect(engine.shouldReason(
      ...
      [{
        ...
        blockers: [], dependencies: [], estimatedCompletion: null,
  ...
  it("should generate rule-based thoughts for errors", async () => {
    const engine = new DefaultReasoningEngine(mockGenerator);
    const result = await engine.cycle(makeContext({
      worldState: {
        project: null, application: DEFAULT_APPLICATION_STATE,
  ...
  it("should generate pattern recognition for multiple observations", async () => {
    ...
    const result = await engine.cycle(makeContext({
      recentObservations: [
        { id: "1", source: "terminal", title: "cmd1", detail: "", priority: "low", score: 10, timestamp: Date.now(), mergeable: false, consumed: false },
        { id: "2", source: "terminal", title: "cmd2", detail: "", priority: "low", score: 10, timestamp: Date.now(), mergeable: false, consumed: false },
        { id: "3", source: "terminal", title: "cmd3", detail: "", priority: "low", score: 10, timestamp: Date.now(), mergeable: false, consumed: false },
      ],
    }));
    const patterns = result.thoughts.filter((t) => t.type === "pattern_recognition");
  ...
  it("should emit thoughts via handler", async () => {
    ...
    await engine.cycle(makeContext({
      worldState: {
        project: null, application: DEFAULT_APPLICATION_STATE,
        system: { ...DEFAULT_SYSTEM_STATE, openErrors: [{ source: "build", message: "fail", timestamp: Date.now() }] },
        timestamp: Date.now(), version: 0,
