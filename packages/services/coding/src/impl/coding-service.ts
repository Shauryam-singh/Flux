...
export function createCodingService(options: CodingServiceOptions): Service {
  ...
  return {
    name: "coding",
    description:
      "Code assistant for writing, editing, reading files, running commands, and git operations",
    ...
    async execute(
      ...
      let toolCall: { tool: string; input: Record<string, unknown> } | null =
        null;
      try {
        const cleaned = text
          .replace(/^```json\s*/i, "")
      ...
      const output =
        resultObj.output ?? (resultObj.success ? "Done" : "Failed");
