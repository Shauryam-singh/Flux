...
import type { TTSEngine, TTSSynthesizeOptions } from "../interfaces/tts-engine.js";
...
export class PiperEngine implements TTSEngine {
  ...
  private async synthesizeWithPiper(text: string, voice?: string): Promise<Buffer> {
    const cacheDir = getCacheDir();
  ...
  private async synthesizeWithEspeak(text: string, voice?: string, speed?: number): Promise<Buffer> {
    const cacheDir = getCacheDir();
    const wavPath = join(cacheDir, "tts_output.wav");
